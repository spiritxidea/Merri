...
function cryout_loadsettings() {
	check_admin_referer( 'cryout-special-string', 'fluida_settings_nonce' );
	...
	if (!empty($_POST['fluida_settings'])) {
		$data = str_replace('&#039;','\'',rawurldecode( trim($_POST['fluida_settings'])) );
		$data = @json_decode($data, TRUE);
		if (is_array($data) && isset($data['fluida_db']) && (0.9 == (float)$data['fluida_db']) ) {
			delete_option( 'fluida_settings' );
			if (update_option( 'fluida_settings', $data)) {
				die('OK');				
			} else {
