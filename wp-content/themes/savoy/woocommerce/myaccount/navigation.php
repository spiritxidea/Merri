    ...
    <div class="nm-MyAccount-user">
        ...
        <div class="nm-MyAccount-user-info">
            <span class="nm-username">
                <?php
                    echo sprintf( esc_attr__( 'Hello %s%s%s (not %2$s? %sSign out%s)', 'woocommerce' ), '<strong>', esc_html( $current_user->display_name ), '</strong><span class="hide">', '<a href="' . esc_url( wc_get_endpoint_url( 'customer-logout', '', wc_get_page_permalink( 'myaccount' ) ) ) . '">', '</a>' ) . '</span>';
                ?>
            </span>
