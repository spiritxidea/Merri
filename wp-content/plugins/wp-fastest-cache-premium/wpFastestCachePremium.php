...
*/
	...
	add_action('wp_ajax_wpfc_save_server_location_ajax_request', "wpfc_save_server_location_ajax_request");

	function wpfc_save_server_location_ajax_request(){
		if(in_array($_GET["location"] , array("de", "us"))){
			$location = get_option("wpfc_server_location");

			if($location){
				update_option("wpfc_server_location", $_GET["location"]);
			}else{
				add_option("wpfc_server_location", $_GET["location"], null, "yes");
			}
